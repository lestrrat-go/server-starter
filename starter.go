	...
	for {
		...
		for i, l := range s.listeners {
			if l.fd >= 0 {
				if l.fd < 3 {
					s.mu.RUnlock()
					panic(fmt.Sprintf("listener descriptor %d conflicts with standard streams", l.fd))
				}
				if _, ok := used[l.fd]; ok {
					s.mu.RUnlock()
					panic(fmt.Sprintf("listener descriptor %d is specified more than once", l.fd))
				}
				descriptors[i] = l.fd
				used[l.fd] = struct{}{}
			}
			...
			default:
				if l.packet != nil {
					if packet, ok := l.packet.(*net.UDPConn); ok {
						...
						err = fmt.Errorf("unknown packet listener type")
					}
				} else {

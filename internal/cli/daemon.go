...
const (
	...
	// Startup errors are bounded by the operating system's command-line and
	// environment limits. This larger protocol bound keeps malformed frames
	// from forcing an unbounded allocation while preserving every practical
	// startup error, including errors over 64 KiB.
	maxDaemonStatusBodySize = 16 << 20
)
...
func readDaemonStatus(reader io.Reader) error {
	...
	case daemonReady:
		...
		return nil
	case daemonFailed:
		if bodySize > maxDaemonStatusBodySize {
			return fmt.Errorf("daemon startup status length exceeds limit")
		}
